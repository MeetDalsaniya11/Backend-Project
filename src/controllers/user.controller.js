...
const registerUser = asyncHandler(async (req, res) => {
    ...
    const existedUser = await User.findOne({
        $or: [{ username }, { email }]             //$or is an Operator
    })
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const { accessToken, refreshToken } = await generateAccessAndRefreshTokens(user._id)      //access and refresh token
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res
            .status(200)
            .cookie("accessToken", accessToken, options)
            .cookie("refreshTooken", newRefreshToken, options)
            .json(
                new ApiResponse(
                    200,
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        //count subscribers by using channel id in subscription collection
        {
            $lookup: {
                from: "subscriptions",
                localField: "_id",
